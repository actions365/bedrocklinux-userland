...
vendor/musl/.success_fetching_source:
	...
	mkdir -p vendor/musl
	git clone --depth=1 \
		-b `git ls-remote --tags 'https://repo.or.cz/musl.git' | \
		awk -F/ '{print $$NF}' | \
		sed 's/^v//g' | \
		grep '^[0-9.]*$$' | \
		sort -t . -k1,1n -k2,2n -k3,3n -k4,4n -k5,5n | \
		tail -n1 | \
		sed 's/^/v/'` 'https://repo.or.cz/musl.git' \
		vendor/musl
...
vendor/busybox/.success_retrieving_source:
	...
	git clone --depth=1 \
		-b `git ls-remote --heads 'https://github.com/mirror/busybox' | \
		awk -F/ '$$NF ~ /stable$$/ {print $$NF}' | \
		sort -t _ -k1,1n -k2,2n -k3,3n -k4,4n -k5,5n | \
		tail -n1` 'https://github.com/mirror/busybox' \
		vendor/busybox
...
vendor/zlib/.success_retrieving_source:
	rm -rf vendor/zlib/
	mkdir -p vendor/zlib
	cd vendor/zlib && wget -O- 'https://zlib.net/fossils/zlib-1.3.1.tar.gz' | gunzip | tar xf -
